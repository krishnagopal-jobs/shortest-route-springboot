...
//    private final List<Node> nodes;
    ...
    public ShortestPathFinderAlgorithm(Graph graph, Node origin) {
    ...
    private int getDistance(Node node, Node target) {
        for (Edge edge : edges) {
            if (edge.getOrigin().equals(node)
                ...
                return edge.getWeightage();
            }
        }
        throw new RuntimeException("There must a programming error");
    }
    ...
    private List<Node> getNeighbors(Node node) {
        ...
        for (Edge edge : edges) {
            if (edge.getOrigin().equals(node)
                    && !settledNodes.contains(edge.getDestination())) {
